...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, split='train', vocab=None, tokenized_captions=None,
        ...
        # Create tokenized captions if there are not there, save them to pickle files.
        if tokenized_captions is None:
            tokenized_captions = cls.get_tokenized_captions(self.caption_path,
                    self.tokens_path)

        # Create the vocabulary and save it. If there exists one, use the existing. It must be a vocabulary object.
        if vocab is None:
            if split != 'train':
        ...
        # Set some variables for later use depending on split.
        if split == 'train':
        ...
        if self.ids_based_on == self.ID_BASE.CAPTIONS:
            # Create a list with ids that are the keys of the annotations.
